...
class Restriction {
	constructor(restrict) {
		...
		this.checks = [ ];
		restrict.hasOwnProperty('from') && this.checks.push(value => value < from && ('This value must be at least '+ from));
		restrict.hasOwnProperty('to') && this.checks.push(value => value > to && ('This value can be at most '+ to));
		restrict.hasOwnProperty('match') && this.checks.push(value => !match.test(value) && (message ? message : ('This value must match '+ match)));
		return Object.freeze(this);
	}
	validate(value) {
